...
def wrapper(args):
    ...
    real_baseDF = real_baseDF[real_baseDF["strand"].isin(["+","-"])].reset_index(drop=True)
    ...
    nonint_baseDF = nonint_baseDF[nonint_baseDF["strand"].isin(["+","-"])].reset_index(drop=True)
    ...
    int_baseDF = int_baseDF[int_baseDF["strand"].isin(["+","-"])].reset_index(drop=True)
    ...
    pol_baseDF = pol_baseDF[pol_baseDF["strand"].isin(["+","-"])].reset_index(drop=True)
    ...
    for i in range(num_samples):        
        ...
        tdf["tpm"] = exps_int
        tdf["tpm"] = tdf["tpm"].fillna(0)
        assert len(tdf[tdf["tpm"]==0])==0,"missing TPMs"
        ...
        tdf.drop(["_merge"],axis=1,inplace=True)
        ...
        tdf["tpm"] = exps_pol
        tdf["tpm"] = tdf["tpm"].fillna(0)
        assert len(tdf[tdf["tpm"]==0])==0,"missing coverages"
        # tdf["nr"] = tdf["elen"]*tdf["cov"]
    ...
    if args.type == "rsem":
        ...
        for i in range(num_samples):
            ...
            print(i,len(tpm_contrib_real),len(tpm_contrib_nonint),len(tpm_contrib_int),len(tpm_contrib_pol))
            ...
            print("number of real reads: "+str(real_nr))
            ...
            print("number of nonint reads: "+str(nonint_nr))
            ...
            print("number of int reads: "+str(int_nr))
            ...
            print("number of pol reads: "+str(pol_nr))
...
# Strategies for dealing with polyester
# 1. try identifying a sigle example to replicate the issue
# 2. try removing fragment length (==readlen and sd=25)
# 3. try removing strand_specific flag off
# 4. try using simulate_experiment_countmat instead
# 5. are there any noticeable properties to the missing coverage?
#     - <=readlen
#     - always at the end or on both ends?
# 6. can we extend 3' and 5' ends to compensate for it and justify it as readthrough/gene-associated transcription